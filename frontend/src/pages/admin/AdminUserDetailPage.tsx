...
function HistoryCard({ item, index, onOpenImage }: HistoryCardProps) {
  ...
  const hasQuery = !!(item.queryText || item.queryImagePath)
  const hasResult = !!item.clickedImage
            ...
            <div className="flex items-center gap-3 flex-wrap sm:flex-nowrap">
                ...
                {item.queryImagePath ? (
                  ...
                  <div className="flex items-center gap-3">
                    <ImageThumb
                      ...
                      aspectRatio=""
                      onOpen={() => onOpenImage(item.queryImagePath!, 'Ảnh tìm kiếm')}
                    />
                    <div className="min-w-0">
                      <p className="text-xs font-medium text-foreground">Tìm bằng ảnh</p>
                      <p className="text-[11px] text-muted-foreground mt-0.5 font-mono truncate">
                        {item.queryImagePath.split('/').pop()}
                      </p>
                    </div>
                  </div>
                ) : item.queryText ? (
              ...
              {hasResult && (
                <div className="flex items-center justify-center shrink-0 self-center">
                  <div className="flex items-center justify-center size-7 rounded-full bg-muted/60 border border-border/40">
              ...
              {hasResult && item.clickedImage && (
                <div className="shrink-0">
                  ...
                  <div className="flex items-center gap-3">
                    <ImageThumb
                      src={item.clickedImage.imageUrl}
                      alt="Clicked image"
                      className="w-24 h-20 shrink-0"
                      aspectRatio=""
                      onOpen={() => onOpenImage(item.clickedImage!.imageUrl, 'Ảnh đã click')}
                    />
                    <div className="min-w-0 hidden sm:block">
                      <p className="text-xs font-medium text-foreground">Đã xem ảnh</p>
                      {(item.clickedImage.width > 0 || item.clickedImage.height > 0) && (
                        <p className="text-[11px] text-muted-foreground mt-0.5">
                          {item.clickedImage.width}×{item.clickedImage.height}
                        </p>
                      )}
                    </div>
                  </div>
              ...
              {!hasResult && (
                <div className="shrink-0">
                  ...
                  <div className="flex items-center justify-center w-24 h-20 rounded-lg border border-dashed border-border/50 bg-muted/20">
                    <p className="text-[10px] text-muted-foreground text-center px-1">Không click ảnh</p>
                  </div>
                </div>
              )}
            </div>
...
export function AdminUserDetailPage() {
  ...
  const { data, isLoading, isError, refetch } = useQuery({
    ...
    retry: false, // Không retry khi backend lỗi 500
  })
  ...
  function goToPage(p: number) {
    navigate({
      ...
      params: { userId },
      search: { page: p, searchType: activeType },
    })
  }
  ...
  function changeType(type: SearchTypeKey) {
    navigate({
      to: '/admin/users/$userId',
      params: { userId },
      search: { page: 1, searchType: type },
    })
  }

  // Short user id display
